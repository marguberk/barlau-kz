...
echo "🔄 Выполняем простую замену данных в продакшн базе BARLAU.KZ"
echo "📅 Дата: $(date)"
...
# Выполняем простую замену данных на сервере
sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no "$USER@$SERVER" << 'EOF'
# Создаем директорию для скриптов
mkdir -p /home/ubuntu/cleanup_scripts

# Создаем простой Python скрипт замены данных
cat > /home/ubuntu/cleanup_scripts/simple_data_replacement.py << 'PYTHON_EOF'
#!/usr/bin/env python3
import os
import sys
import django
...
def simple_data_replacement():
    print("🔄 Начинаем простую замену данных в продакшн базе...")
    ...
    with transaction.atomic():
        ...
        real_employees = [
            ('Серік', 'Айдарбеков', 'serik_aydarbekov', 'MANAGER', '+77757599686', 'serik@barlau.org', 'Родился 1 июня 1986 года. ИИН: 890601301168'),
            ('Алмасжан', 'Сопашев', 'almaszhan_sopashev', 'MANAGER', '+77057057876', 'almaszhan@barlau.org', 'Родился 24 декабря 1988 года в г.Жаркент. Семейное положение-Женат. Окончил Международный университет «Silkway». ИИН: 881224302564'),
            ('Мақсат', 'Құсайын', 'maksat_kusayin', 'MANAGER', '+77014977888', 'maksat@barlau.org', 'Родился 16 мая 1988 года в г.Жаркент. Семейное положение-Женат. ИИН: 880516300512'),
            ('Ерболат', 'Кудайбергенов', 'erbolat_kudaybergenov', 'MANAGER', '+77017525437', 'erbolat@barlau.org', 'Родился 25 февраля 1978 года с.Елтай, Ерейментауский р-он, Акмолинская область. ИИН: 780225350070'),
            ('Назерке', 'Садвакасова', 'nazerke_sadvakasova', 'ACCOUNTANT', '+77023200506', 'nazerke@barlau.org', 'Родилась 1 августа 1992 года в г.Жаркент. Семейное положение-Замужем. Окончила КазНУ'),
            ('Ғабит', 'Ахметов', 'gabit_akhmetov', 'TECH', '+77071778202', 'gabit@barlau.org', 'Родился 13 декабря 1990 года в г.Жаркент. Семейное положение-Холостой. ИИН: 901213302223'),
            ('Азиз', 'Илямов', 'aziz_ilyamov', 'TECH', '+777789509264', 'aziz@barlau.org', 'Родился 22 октября 1995 года в г.Жаркент. Семейное положение-Женат. ИИН: 951022300812'),
            ('Айдана', 'Ұзақ', 'aidana_uzak', 'MANAGER', '+77076156475', 'aidana@barlau.org', 'Родилась 1 февраля 1998 года в г.Жаркент. 2020 году окончила АТУ, по специальности-технология. Семейное положение-Замужем. Дети-1. ИИН: 980201400864'),
            ('Асель', 'Мұрат', 'asel_murat', 'MANAGER', '+777781008373', 'asel@barlau.org', 'Родилась 19 сентября 1995 года в г.Жаркент. Образование-незаконченное высшее. Семейное положение-в разводе. Дети-1. ИИН: 950919401325')
        ]
        ...
        for emp_data in real_employees:
            try:
                cursor.execute("""
                    ...
                    (username, first_name, last_name, email, role, phone, about_me, is_active, is_staff, is_superuser, password, date_joined, last_login)
                    VALUES (?, ?, ?, ?, ?, ?, ?, 1, 0, 0, 'pbkdf2_sha256$600000$dummy$dummy', datetime('now'), NULL)
                """, emp_data)
                added_employees += 1
        ...
        for driver_data in real_drivers:
            try:
                cursor.execute("""
                    ...
                    (username, first_name, last_name, email, role, phone, about_me, is_active, is_staff, is_superuser, password, date_joined, last_login)
                    VALUES (?, ?, ?, ?, ?, ?, ?, 1, 0, 0, 'pbkdf2_sha256$600000$dummy$dummy', datetime('now'), NULL)
                """, (driver_data[2], driver_data[0], driver_data[1], f"{driver_data[0].lower()}@barlau.org", 'DRIVER', driver_data[3], f"Водительское удостоверение: {driver_data[4]}"))
                added_drivers += 1
        ...
        for vehicle_data in real_vehicles:
            try:
                cursor.execute("""
                    ...
                    (license_plate, model, type, is_active, year, capacity, fuel_type, status, created_at, updated_at)
                    VALUES (?, ?, ?, 1, 2020, 20.0, 'DIESEL', 'AVAILABLE', datetime('now'), datetime('now'))
                """, vehicle_data)
                added_vehicles += 1
        ...
        print(f"\n🎉 Простая замена данных завершена!")
...
BACKUP_FILE="backup_before_simple_replacement_$(date +%Y%m%d_%H%M%S).sqlite3"
cp db.sqlite3 "/home/ubuntu/backups/$BACKUP_FILE"
echo "✅ Резервная копия создана: $BACKUP_FILE"

# Выполняем простую замену данных
echo "🔄 Выполняем простую замену данных..."
python /home/ubuntu/cleanup_scripts/simple_data_replacement.py

echo "✅ Простая замена данных завершена!"
EOF

echo ""
echo "🎉 Простая замена данных в продакшн базе завершена!"
