...
# Финальный исправленный скрипт для полной замены данных на продакшн сервере
...
from logistics.models import Vehicle, Trip, Expense
...
def complete_data_replacement():
    ...
    with transaction.atomic():
        ...
        # Список пользователей для удаления (обычные сотрудники)
        users_to_remove = [
        ...
        for user_name in users_to_remove:
            try:
                user = User.objects.filter(
                    first_name__icontains=user_name.split()[0],
                    last_name__icontains=user_name.split()[1]
                ).first()
                
                if user:
                    print(f"  ❌ Удаляем: {user_name}")
                    user.delete()
                    removed_users += 1
                else:
                    print(f"  ⚠️  Не найден: {user_name}")
        ...
        for vehicle_data in real_vehicles:
            try:
                ...
                if created:
                    ...
                    print(f"  ⚠️  ТС уже существует: {vehicle_data['license_plate']}")
