#!/usr/bin/env python3
"""
Синхронизация GPS данных из StavTrack API с грузовиками на продакшн сервере
"""
import os
import django
import time
...
def sync_stavtrack_to_production():
    """
    Синхронизируем GPS данные из StavTrack с грузовиками на продакшн сервере
    """
    print("🔄 СИНХРОНИЗАЦИЯ STAVTRACK → ПРОДАКШН")
    print("=" * 60)
    ...
    # Получаем все грузовики из продакшн базы
    all_vehicles = Vehicle.objects.all().order_by('number')
    print(f"\n🚛 Найдено {all_vehicles.count()} грузовиков в продакшн базе:")
    ...
    for stavtrack_unit in stavtrack_units:
        ...
        print(f"\n🔍 Ищем грузовик {stavtrack_name} в продакшн базе...")
        ...
        if matching_vehicle:
            ...
            if position:
                ...
                # Обновляем GPS данные в продакшн базе
            ...
            print(f"   ❌ Грузовик {stavtrack_name} не найден в продакшн базе")
    
    print(f"\n📊 ИТОГИ СИНХРОНИЗАЦИИ:")
    print(f"   ✅ Обновлено грузовиков: {updated_count}")
    print(f"   📡 Устройств в StavTrack: {len(stavtrack_units)}")
    print(f"   🚛 Грузовиков в продакшн базе: {all_vehicles.count()}")
...
if __name__ == "__main__":
    sync_stavtrack_to_production()
    print("\n✅ Синхронизация StavTrack → Продакшн завершена!")
